...
public interface BuildLink {
   ...
   *
   * <p>This can be used by Play plugins, for example, by a test fixture plugin to run tests. The
   * format of the passed in task and the return value are build tool specific.
   *
   * <p>For the default SBT implementation, it's a standard SBT task, and the result is the return
   * value of that task. Note that if the return value is anything but JDK classes, the only way to
   * access it will be using reflection.
   *
